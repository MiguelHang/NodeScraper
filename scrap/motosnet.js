...
function retrieveAds(model, location, brand, callback) {
    let options =  {
        ...
        url: `https://motos.coches.net/ocasion/${brand.replace(/ /g, '-')}/${model.replace(/ /g, '_')}/${location.replace(' ', '-')}&fi=SortDate`,
        headers: {
            'User-Agent': 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:41.0) Gecko/20100101 Firefox/41.0',
            'Accept': '*/*',
            'Accept-Language': 'es-ES,es;q=0.8,en-US;q=0.5,en;q=0.3',
            'Referer': `https://motos.coches.net/ocasion/${brand.replace(/ /g, '-')}/${model.replace(/ /g, '_')}/${location.replace(' ', '-')}&fi=SortDate`,
        }
    };
    ...
    request(options, function(err, resp, body) {
        ...
        $('#rows .lnkad').each(function() {
            ...
            let img = $(this).find('.p').attr('data-iurl') || $(this).find('.p').attr('style').split(/['']/)[1];
